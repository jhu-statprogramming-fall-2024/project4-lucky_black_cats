...
wiki_data <- get_wiki_pageviews("Tesla, Inc.", "2024-01-01", "2024-01-20")
stock_data <- get_stock_data("TSLA", "2024-01-01", "2024-01-20")
...
train_and_test <- function(data) {
  # Frist six days are training
  train_data <- data[1:6, ]
  
  # The seventh day is testing (MAKE SURE THIS ISN'T ALWAYS THE SAME WEEKDAY BY VARYING DATES)
  test_data <- data[7, ]
  
  # We're going to use a logistic regression model (simple UP or DOWN) - this is what truly matters to investors
  model <- train(
    Price_Change ~ ., 
    data = train_data[, c("Price_Change", paste0("Pageviews_lag", 1:7))],   #These are actually the LAGGED dates
    method = "glm", 
    family = "binomial"
  )
  
  # Predict for the seventh day
...
```{r}
stocks <- c("TSLA", "AAPL")
wiki_pages <- c("Tesla, Inc.", "Apple_Inc.")
